...
- 이후 사용자가 원본 검증 범위를 645장 전체로 확대했다. 이번 작업은 기존 1~80장 결과를 재사용하고 81~645장 원본을 검사·수정한다. 이미 발견한 가사 차이는 추가 대조한다. 서버로 보낼 원본 ZIP과 수정 기록이 산출물이며, 앱 변경·전곡 PDF/PNG 생성은 이번 작업 범위가 아니다.
- 서버에서 645곡의 원래 조 생성 완료 후, 사용자가 모든 지원 조의 사전 생성을 요청했다. `pnpm run prerender`는 기본 645곡 × 13개 조를 준비한다. 전체 생성은 서버에서 실행하며 로컬에서 전곡 렌더하지 않는다. 등록된 곡은 영속 작업 큐와 앱의 조 목록에 함께 반영한다.
